...
状態: **実装済み**．4 章のステップ 1〜3 を feat/issue3-concurrent-startup で実装した．
実装時に確認した実測値は 8 章にまとめている．
更新: 2026-09-07
...
3. ~~本計画のレビューとマージ~~ → PR #7 でマージ済み（2026-09-07）．
4. ~~4 章のステップ 1 から着手する~~ → ステップ 1〜3 を実装（2026-09-07）．
5. Linux ネイティブ / WSL2 での同時起動は未確認のまま（手元に環境が無い）．

## 8. 実装時に確認したこと（2026-09-07）

6 章の未確認項目のうち，実装時に決着したものを記録する．

**同時起動は実機で通った**

`podman machine` を 18432MiB / 60GB へ拡張し，サンプル DB → OpenMetadata → DataHub の
順に起動して 13 コンテナの同時稼働を確認した．両ツールの `status.sh` が
「正常に稼働している」を返し，`ingest.sh` も両方が検証まで通った
（詳細な実測値は `docs/comparison.md` の「ポート衝突とリソース（同時起動）」）．

| 項目 | 実測 |
| --- | --- |
| 起動所要（イメージ pull 済み） | OpenMetadata 45 秒 / DataHub 53 秒 |
| 実メモリ使用量の合計 | 約 10.4GB（`mem_limit` 合計 15.5g に対して） |
| ホスト公開ポート | 3.1 の表どおり．重複なし |
| CPU | 4 のままで実用上の問題なし（3.6 の判断どおり，増やさない） |

**`status.sh` のラベル衝突は実在した**

`com.docker.compose.service=mysql` は OpenMetadata 側のコンテナにも付く
（`container_name` を明示していても compose のラベルは付与される）．
`podman ps` の出力で確認し，3.5 のとおり project ラベルとの併用に直した．

**`!override` の統合後も期待どおりマージされた**

`podman compose config` の出力で，両スタックとも重複や upstream の残留がないことを確認した．
OpenMetadata は 8585/8586・18080・13306・19200/19300，
DataHub は 9002・8080/4319・9092・23306・29200．

**podman machine のメモリ不足は想定どおりの導線になった**

拡張前（9536MiB）に同時起動用の要求値（16384MiB）でチェックすると，
`podman machine stop / set --memory / start` の手順を案内して停止することを確認した．

**まだ未確認のまま残るもの**

- Linux ネイティブ / WSL2 での同時起動（machine 非経由の経路）．
- 初回のイメージ pull を含めた起動時間（今回は pull 済みの状態での計測）．
- 長時間稼働させた場合のメモリ挙動（計測は起動直後の 1 点のみ）．
